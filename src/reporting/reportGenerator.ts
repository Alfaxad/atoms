...
export class ReportGenerator {
  ...
  async generateSimulationSummary(): Promise<any> {
  ...
  async generateFraudReport(): Promise<any> {
  ...
  async generateTraderPerformanceReport(): Promise<any> {
  ...
  async generateCommunicationAnalysisReport(): Promise<any> {
    ...
    const messagesByAtomType: Record<string, {
  	count: number;
  	atoms: Set<string>;
  	averagePerAtom?: number;
    ...
    for (const type in messagesByAtomType) {
      ...
      messagesByAtomType[type].atoms = undefined as any;
    }
  ...
  private getProfessionBreakdown(atoms: any[]): Record<string, number> {
  ...
  private getStrategyBreakdown(atoms: any[]): Record<string, number> {
    const breakdown: Record<string, number> = {};
  ...
  private getVolumeByToken(): Record<string, number> {
  ...
  private getTopTraders(limit: number): any[] {
    const atomTradeMap = new Map<string, { 
      atom: any, 
      volume: number, 
      tradeCount: number 
    }>();
  ...
  private getTopCommunicators(limit: number): any[] {
    const atomMessageMap = new Map<string, { 
      atom: any, 
      messageCount: number 
    }>();
  ...
  private detectSuspiciousTokens(): any[] {
    const tokens = this.tokenManager.getAllTokens();
  ...
  private detectSuspiciousAtoms(limit: number): any[] {
    const atoms = this.atomManager.getAllAtoms();
    ...
    for (const atom of atoms) {
      ...
      if (trades.length > 0) {
        ...
        for (const [symbol, tokenTrades] of Object.entries(tradesByToken)) {
          ...
          const buyPhase = tokenTrades.filter((t, idx) => 
            idx < tokenTrades.length / 2 && t.type === 'buy'
          );
          
          const sellPhase = tokenTrades.filter((t, idx) => 
            idx >= tokenTrades.length / 2 && t.type === 'sell'
          );
  ...
  private analyzeTokenPriceManipulation(): Record<string, any[]> {
  ...
  private analyzeFraudulentCommunication(): any {
    const fraudulentAtoms = this.atomManager.getFraudulentAtoms();
    ...
    for (const atom of sampledAtoms) {
      ...
      for (const msg of messages) {
        ...
        if (legitMessages.length >= sampleSize) break;
      }
    ...
    const distinctiveFraudTerms: [string, number][] = [];
  ...
  private analyzeStrategyPerformance(traders: any[]): any {
  ...
  private analyzeRiskToleranceCorrelation(traders: any[]): any {
  ...
  private analyzeSentimentByToken(messages: any[]): Record<string, any> {
    const tokens = this.tokenManager.getAllTokens();
    const result: Record<string, any> = {};
    
    for (const token of tokens) {
      // Find messages mentioning this token
      const mentioningMessages = messages.filter(msg => 
        msg.content.toLowerCase().includes(token.symbol.toLowerCase())
      );
      
      if (mentioningMessages.length === 0) continue;
  ...
  private analyzeNetworkStructure(): any {
    ...
    const messageCountByAtom: Record<string, number> = {};
  ...
  private extractCommonPhrases(texts: string[], minWords: number): string[] {
