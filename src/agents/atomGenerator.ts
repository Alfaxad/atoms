import { AtomPersona, AtomProfile } from './types';
import { PersonaGenerator } from './personaGenerator';
import { WalletGenerator } from './walletGenerator';
import { v4 as uuidv4 } from 'uuid';
import * as fs from 'fs';
import * as path from 'path';
import { DECISION_MODELS, TRADING_STRATEGIES } from '../config/constants';
...
export class AtomGenerator {
  ...
  private assignTradingStrategy(fraudulent: boolean): string {
    if (fraudulent) {
      // Assign fraudulent strategy
      return Math.random() < 0.5 ? 
        TRADING_STRATEGIES.PUMP_AND_DUMP : 
        TRADING_STRATEGIES.RUG_PULL;
    } else {
      // Assign legitimate strategy
      const strategies = [
        TRADING_STRATEGIES.VALUE_INVESTING,
        TRADING_STRATEGIES.MOMENTUM_TRADING,
        TRADING_STRATEGIES.SWING_TRADING,
        TRADING_STRATEGIES.DAY_TRADING,
        TRADING_STRATEGIES.ARBITRAGE,
      ];
      return strategies[Math.floor(Math.random() * strategies.length)];
    }
  }

  private assignDecisionModel(): string {
    const models = Object.values(DECISION_MODELS);
    return models[Math.floor(Math.random() * models.length)];
  }

  private calculateRiskTolerance(persona: AtomPersona): number {
  ...
  private determineInitialBalance(persona: AtomPersona): number {
  ...
  async generateAtom(fraudulent: boolean = false): Promise<AtomProfile> {
  ...
  private saveAtomData(atom: AtomProfile): void {
    const filePath = path.join(this.basePath, `${atom.id}.json`);
    fs.writeFileSync(filePath, JSON.stringify(atom, null, 2));
  }

  async generateBatch(count: number, fraudPercentage: number = 0.05): Promise<AtomProfile[]> {
    const atoms: AtomProfile[] = [];
