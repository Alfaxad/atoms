...
exports.SimulationMonitor = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class SimulationMonitor {
    constructor(simulationManager, tokenManager, marketSimulator, tradingSystem, communicationSystem) {
    ...
    startMonitoring(snapshotFrequencyMs = 60000) {
    ...
    generateReport() {
        try {
            ...
            const report = {
                ...
                marketSummary: {
                    tokens: latestSnapshot.market.tokens.map((token) => ({
                        symbol: token.symbol,
                        currentPrice: token.price,
                        priceChange: this.calculatePriceChange(firstSnapshot, latestSnapshot, token.symbol)
                    })),
                    tradingVolume: latestSnapshot.trading.recentTrades.reduce((sum, trade) => sum + (trade.amount * trade.price), 0)
                },
    ...
    calculatePriceChange(firstSnapshot, latestSnapshot, symbol) {
        try {
            const firstToken = firstSnapshot.market.tokens.find((t) => t.symbol === symbol);
            const latestToken = latestSnapshot.market.tokens.find((t) => t.symbol === symbol);
            if (!firstToken || !latestToken)
                return 0;
...
exports.SimulationMonitor = SimulationMonitor;
