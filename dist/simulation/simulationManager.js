...
class SimulationManager {
    constructor(config) {
        ...
        this.atomManager = new atomManager_1.AtomManager();
        this.tokenManager = new tokenManager_1.TokenManager();
        this.communicationSystem = new communicationSystem_1.CommunicationSystem(this.atomManager);
    ...
    async initialize() {
        ...
        this.marketSimulator = new marketSimulator_1.MarketSimulator(this.tokenManager);
        await this.marketSimulator.loadMarketState();
        // Initialize trading system
        this.tradingSystem = new tradingSystem_1.TradingSystem(this.tokenManager, this.marketSimulator, this.atomManager);
    ...
    async generateAtomActivity(simTimeDelta) {
        ...
        if (tradingAtomCount > 0) {
            ...
            for (const atom of tradingAtoms) {
                try {
                    ...
                    // Execute trade
                    const trade = await this.tradingSystem.executeTrade(atom.id, tokenSymbol, tradeType, amount);
                    // Generate message about the trade
                    await this.communicationSystem.generateTradeMessage(atom.id, tradeType, tokenSymbol, amount, this.marketSimulator.getTokenPrice(tokenSymbol));
                }
                catch (error) {
        ...
        if (messageAtomCount > 0) {
            ...
            for (const atom of messageAtoms) {
                try {
                    ...
                    // Generate message
                    await this.communicationSystem.generateMarketCommentMessage(atom.id, tokenSymbol, priceChange);
                }
                catch (error) {
                    // Silently continue
                }
        ...
        if (fraudulentAtoms.length > 0) {
            ...
            if (Math.random() < fraudProbability) {
                ...
                await this.communicationSystem.generateFraudCampaignMessages(targetToken, fraudPhase);
