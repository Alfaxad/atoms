...
const environment_2 = require("../utils/environment");
async function main() {
    try {
        ...
        if (!atomsToken) {
            ...
            atomsToken = await tokenManager.createToken('ATOMS', 'ATOMS Simulation Token', 9, // 9 decimals (standard for Solana)
            1000000, // 1 million initial supply
            environment_2.env.SOLANA_PRIVATE_KEY // creator (main wallet)
            );
        ...
        if (!usdcToken) {
            ...
            usdcToken = await tokenManager.createToken('USDC', 'USD Coin (Simulated)', 6, // 6 decimals (standard for USDC)
        ...
        for (const tokenInfo of additionalTokens) {
            ...
            if (!token) {
                ...
                token = await tokenManager.createToken(tokenInfo.symbol, tokenInfo.name, 9, // 9 decimals
                500000, // 500k initial supply
                environment_2.env.SOLANA_PRIVATE_KEY // creator
                );
        ...
        if (atoms.length > 0) {
            ...
            for (const atom of randomAtoms) {
                try {
                    ...
                    const trade = await tradingSystem.executeTrade(atom.id, tokenSymbol, tradeType, amount);
